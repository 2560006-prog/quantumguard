      ...
      <div className="f-sidebar">
        ...
        <nav className="f-nav">
          {NAV.map((item, i) => (<>
            ...
            <div key={item.id} className={'f-nav-item' + (activePage === item.id ? ' active' : '')} onClick={() => showPage(item.id)} style={{ cursor: 'pointer' }}>
          ...
          <div className="f-page active">
            ...
            <div className="f-content">
              <div style={{ marginBottom: '14px', padding: '12px 16px', borderRadius: '10px', border: `1px solid ${sBorder}`, background: sBg, fontSize: '13px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <strong style={{ color: sColor }}>Verification: {verStatus.replace('_', ' ').toUpperCase()}</strong>
          ...
          <div className="f-page active">
            <div className="f-topbar">
              ...
              <div className="f-topbar-right">
                ...
                {editMode && <button className="btn btn-primary" style={{ padding: '6px 14px', fontSize: '12px', background: 'linear-gradient(135deg,#4CAF50,#2e7d32)' }} onClick={saveProfile}>💾 Save</button>}
              </div>
            </div>
            <div className="f-content">
              {/* Profile Header */}
              <div className="profile-header" style={{ marginBottom: '16px' }}>
                <div style={{ position: 'relative' }}>
                  ...
                  <label style={{ position: 'absolute', bottom: '-2px', right: '-2px', background: '#4CAF50', borderRadius: '50%', width: '22px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '11px' }} title="Change photo">
                ...
                <div>
                  ...
                  <div className="profile-verify" style={{ display: 'inline-flex', marginTop: '6px' }}>
              ...
              {!p && !editMode && (
                <div style={{ padding: '32px', textAlign: 'center', background: '#fff3e0', borderRadius: '12px', border: '1px solid #ffe0b2', marginBottom: '16px' }}>
                  ...
                  <p style={{ fontSize: '13px', color: '#6b7280', marginBottom: '14px' }}>Click Edit Profile to fill in your details and start the verification process.</p>
                  <button className="btn btn-primary" style={{ background: 'linear-gradient(135deg,#4CAF50,#2e7d32)' }} onClick={() => { setEditForm({ full_name: user?.full_name || '' }); setEditMode(true); }}>
                  ...
                  <div className="f-card" style={{ gridColumn: '1/-1' }}>
                    ...
                    <div className="profile-grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', gap: '8px' }}>
                      {[['Bank', p.bank_name],['Account Holder', p.account_holder_name],['Account No.', '****' + String(p.account_number || '').slice(-4)],['IFSC', p.ifsc_code]].map(([lbl, val]) => (
                        <div key={lbl} className="profile-field"><div className="profile-field-label">{lbl}</div><div className="profile-field-val" style={{ fontFamily: lbl === 'IFSC' || lbl === 'Account No.' ? 'monospace' : 'inherit' }}>{val || '—'}</div></div>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <>
                  ...
                  <div className="f-card" style={{ marginBottom: '16px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                      ...
                      <div>
                        <label className="btn btn-primary" style={{ padding: '10px 18px', cursor: 'pointer', background: 'linear-gradient(135deg,#4CAF50,#2e7d32)', fontSize: '13px' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginTop: '4px' }}>
                            ...
                            <div style={{ flex: 1 }}>
                              <div style={{ fontSize: '13px', fontWeight: 700, color: '#1a1a1a' }}>
                                {dt.label}
                                <span style={{ marginLeft: '6px', fontSize: '9px', background: isUploaded ? '#e8f5e9' : '#fff3e0', color: isUploaded ? '#2e7d32' : '#e65100', padding: '2px 7px', borderRadius: '10px', fontWeight: 700 }}>
                                  {isUploaded ? `✓ ${uploaded.length} file(s)` : 'Required'}
                                </span>
                              </div>
                              {uploaded.map((d: any) => (
                                <div key={d.id} style={{ marginTop: '6px', padding: '6px 8px', background: '#f1f8f1', borderRadius: '7px', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                  <a href={d.file_url} target="_blank" rel="noreferrer" style={{ color: '#2e7d32', fontWeight: 600, textDecoration: 'none', flex: 1 }}>
                                    📎 {d.document_name}
                                  </a>
                  ...
                  <div style={{ padding: '10px 14px', background: '#f1f8f1', borderRadius: '10px', border: '1px solid #c8e6c9', fontSize: '12px', color: '#6b7280' }}>
                    🔗 All documents are stored securely in <strong style={{ color: '#2e7d32' }}>Supabase Storage</strong>. Click any file name to view it.
                  </div>
                </>
              )}
            </div>
            ...
            <div className="f-content">
                ...
                <div style={{ display: 'grid', gap: '12px' }}>
                  {[
                    ...
                    <div key={label}>
                      ...
                      <div className="chain-hash" style={{ cursor: 'pointer', wordBreak: 'break-all' }} onClick={() => { navigator.clipboard.writeText(val); toast.success('Copied to clipboard!'); }}>{val}</div>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div className="f-card">
                  ...
                  {[
                    ['Block Number', '#7,234,891'],
                    ['Registered On', p?.created_at ? new Date(p.created_at).toLocaleDateString('en-IN') : '—'],
                    ['Gas Used', '62,341 gwei'],
                    ['Status', p ? '✓ Success' : 'Not registered'],
                  ].map(([k, v]) => (
                ...
                <div className="f-card">
                  ...
                  {[
                    ['Contract Address', '0x4a3B...8f2E'],
                    ['Network', 'Sepolia Testnet'],
                    ['Verification', p ? 'Verified ✓' : 'Pending'],
                    ['Registered Farmer', user?.full_name || '—'],
                  ].map(([k, v]) => (
                    <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
        ...
        {activePage === 'loan' && (
          <div className="f-page active">
            <div className="f-topbar">
              ...
              <div className="f-topbar-right">
                <button className="btn btn-primary" style={{ padding: '7px 14px', fontSize: '12px', background: 'linear-gradient(135deg,#4CAF50,#2e7d32)' }} onClick={() => toast.success('💰 Loan application submitted! Bank will contact within 3-5 days.')}>
                  Apply for Loan
                </button>
              </div>
            </div>
            <div className="f-content">
              <div className={`loan-result ${loanEligible ? 'approved' : 'approved'}`}>
                <div className={`loan-result-icon ${loanEligible ? 'approved' : ''}`}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {[
                    ...
                    <div key={loan.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: loan.color === 'green' ? '#f1f8f1' : '#fff3e0', borderRadius: '8px', border: `1px solid ${loan.color === 'green' ? '#c8e6c9' : '#ffe0b2'}` }}>
                      ...
                      <button className={`btn ${loan.color === 'green' ? 'btn-primary' : 'btn-orange'}`} style={{ padding: '6px 14px', fontSize: '11px', ...(loan.color === 'green' ? { background: 'linear-gradient(135deg,#4CAF50,#2e7d32)' } : { background: 'linear-gradient(135deg,#FFA726,#e65100)' }) }} onClick={() => toast.success(`💰 Applied for ${loan.name}!`)}>Apply</button>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '20px', alignItems: 'start' }}>
                <div className="qr-card-wrap" style={{ maxWidth: '280px' }}>
                  ...
                  <div id="qr-canvas-real" style={{ width: '180px', height: '180px', margin: '8px 0' }}>
                    {!farmerId && <div style={{ width: '180px', height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f5f5', borderRadius: '8px', fontSize: '12px', color: '#9e9e9e', textAlign: 'center', padding: '16px' }}>Complete profile to generate QR</div>}
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  <div className="f-card">
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--text2)', padding: '10px 12px', background: 'var(--green-xpale)', borderRadius: '8px', fontFamily: 'monospace', lineHeight: 2, wordBreak: 'break-all' }}>
                      {[['ID', farmerId || '—'],['NAME', user?.full_name || '—'],['STATUS', verStatus.toUpperCase()],['LAND', p ? `${p.land_area}ac` : '—'],['CROP', p?.crop_type || '—'],['DOCS', `${docs.length} uploaded`]].map(([k, v]) => (
        ...
        {activePage === 'status' && (
          <div className="f-page active">
            ...
            <div className="f-content">
              ...
              <div className="f-card" style={{ marginBottom: '14px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {[
                    ...
                    ['Verification Complete', isApproved, isApproved ? `Approved ${v?.reviewed_at ? new Date(v.reviewed_at).toLocaleDateString('en-IN') : ''}` : isRejected ? 'Rejected — see remarks' : 'Awaiting decision'],
                  ].map(([label, done, desc]) => (
                    <div key={label as string} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '8px', background: done ? '#f1f8f1' : '#fafafa', border: `1px solid ${done ? '#c8e6c9' : '#e0e0e0'}` }}>
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: done ? 'var(--green-mid)' : '#e0e0e0', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '13px', flexShrink: 0 }}>
                        {done ? '✓' : '○'}
                      </div>
                      <div style={{ flex: 1 }}>
                      ...
                      <span style={{ fontSize: '10px', background: done ? '#e8f5e9' : '#fff3e0', color: done ? '#2e7d32' : '#e65100', padding: '2px 8px', borderRadius: '10px', fontWeight: 700 }}>
                        {done ? '✓ Done' : 'Pending'}
                      </span>
                    </div>
                  ))}
              ...
              <div style={{ marginTop: '14px', padding: '10px 14px', background: 'var(--green-xpale)', borderRadius: '10px', border: '1px solid #c8e6c9', fontSize: '12px', color: 'var(--text2)' }}>
                ℹ️ Complete all steps to get your <strong style={{ color: 'var(--green)' }}>Blockchain Identity Certificate</strong> and access all government schemes.
              </div>
            </div>
          </div>
        )}
